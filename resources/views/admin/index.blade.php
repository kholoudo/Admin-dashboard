                            ...
                            @foreach ($user->services as $oneservice)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form
                                            action="{{ URL::to('/admin/dashboard/' . $oneservice->pivot->user_id . '/update/' . $oneservice->pivot->id) }}"
                                            method="post">
                                            @csrf
                                            @method('put')
                                            <select class="form-select" name="status" onchange="this.form.submit()">
                                                <option value="{{ $oneservice->pivot->status }}">
                                                    {{ $oneservice->pivot->status }}</option>
                                            ...
                                            <input type="hidden" name="service_id"
                                                value="{{ $oneservice->pivot->service_id }}">
                                            <input type="hidden" name="mobile_number"
                                                value="{{ $oneservice->pivot->mobile_number }}">
                                    ...
                                    <td>
                                        <textarea class="form-control" id="exampleFormControlTextarea3" rows="3"
                                            readonly>{{ $oneservice->pivot->note }}</textarea>
                                    </td>
                                </tr>
                            @endforeach
                        @endforeach
        ...
        <div class="bg-light rounded-top p-4">
            <div class="row">
                <div class="col-12 col-sm-6 text-center text-sm-start">
                    &copy; <a href="#">ٍService Way</a>, All Right Reserved.
                </div>
                <div class="col-12 col-sm-6 text-center text-sm-end">
                    <!--/*** This template is free as long as you keep the footer author’s credit link/attribution link/backlink. If you'd like to use the template without the footer author’s credit link/attribution link/backlink, you can purchase the Credit Removal License from "https://htmlcodex.com/credit-removal". Thank you for your support. ***/-->
                    Designed By <a href="https://htmlcodex.com">Kholoud Khraisat</a>
                </div>
            </div>
        </div>
    </div>
